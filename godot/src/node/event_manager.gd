...
# public methods
func subscribe(_event_name = "", _ref = null, _method = "", _oneshot = false, _val = null):
	var subscribed = false
	var ref_valid = _proc_ref_valid(_event_name, _ref, _method)
	var listener_valid = (
		ref_valid
		if _val == null
		else ref_valid && (_type.built_in_type(_val) or _type.is_type_object(_val))
	)
	if listener_valid:
		var listener_name = _proc_listener_name(_ref)
		if _str.is_valid(listener_name):
			_data.listeners.add(_event_name, listener_name, _ref, _method, _oneshot, _val)
...
func publish(_event_name = "", _val = null):
	...
	if _has_listeners() && _data.listeners.has_event(_event_name):
		if not _val == null:
			...
			_data.event_queue.add(_event_name)
...
func _on_init(_do_init = true):
	...
	if on_do_init or on_do_deinit:
		_data.event_queue = null
		_data.listeners = null
...
func _sub_req_subs_event(_subscribe = true):
	return (
		subscribe(_REQ_SUBS_EVENT, self, _REQ_SUBS_FUNC, false, _NAME)
		if _subscribe
		else unsubscribe(_REQ_SUBS_EVENT, self)
	)


func _sub_req_unsub_event(_subscribe = true):
	return (
		subscribe(_REQ_UNSUB_EVENT, self, _REQ_UNSUB_FUNC, false, _NAME)
		if _subscribe
		else unsubscribe(_REQ_UNSUB_EVENT, self)
	)
